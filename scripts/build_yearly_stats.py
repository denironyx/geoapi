...
def main():
    ...
    try:
        ...
        # path to the parquet file
            ...
            # Task3: Run summary query and write outputs (must run while connection is open)
            logger.info("Creating changesets summary query...")
            unified_query = f"""
            WITH base_stats AS (
                -- Step 1: Basic yearly aggregations
                SELECT
            ...
            first_seen AS (
                -- Step 2: Calculate when each user first appeared
                SELECT uid, min(year(created_at)) AS first_year
                FROM changesets_norm
                GROUP BY uid
            ),

            new_mappers_stats AS (
                -- Step 3: Count new mappers per year
            ...
            editor_ranking AS (
                -- Step 4: Rank editors per year by volume
                SELECT
                    year(created_at) AS year,
                    COUNT(DISTINCT map_editor) AS editors,
                    count(*) AS cnt
                FROM changesets_norm
                WHERE year(created_at) BETWEEN {START_YEAR} AND {END_YEAR} AND map_editor IS NOT NULL
                GROUP BY 1
            )

            -- Final Step: Join everything together
            SELECT 
                b.year,
                b.changesets,
                b.total_edits,
                b.active_mappers,
                n.new_mappers,
                COALESCE(e.editors, 0) AS total_editor
            FROM base_stats b
            LEFT JOIN new_mappers_stats n ON b.year = n.year
            LEFT JOIN editor_ranking e ON b.year = e.year 
            ORDER BY b.year;
            """
            
            
            logger.info("Executing changesets summary query...")
            df_final = con.execute(unified_query).df()
            ...
            for _, r in df_final.iterrows():
                year = str(int(r['year']))
                out[year] = {
                    'changesets': int(r['changesets']) if pd.notna(r.get('changesets')) else None,
                    'contributors': int(r['active_mappers']) if pd.notna(r.get('active_mappers')) else None,
                    'mappedFeatures': int(r['total_edits']) if pd.notna(r.get('total_edits')) else None,
                    # Placeholder: source data doesn't include countries; set to 0 or replace with a computed value
                    'countries': 0,
                    'apps': int(r['total_editor']) if pd.notna(r.get('total_editor')) else None,
